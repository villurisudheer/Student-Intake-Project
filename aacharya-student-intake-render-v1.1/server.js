...
const server=http.createServer(async (req,res)=>{
  ...
  const route=u.pathname.length>1?u.pathname.replace(/\/+$/,''):u.pathname;
  if(req.method==='OPTIONS'){res.writeHead(204,cors());return res.end();}
  if((route==='/api/health'||route==='/health')&&req.method==='GET') return send(res,200,{ok:true,service:'Aacharya Student Information Form',storage:DATA_DIR});
  ...
  if(route==='/api/admin/submissions'&&req.method==='GET'){
  ...
  if(route==='/api'&&req.method==='GET') return send(res,200,{ok:true,service:'Aacharya Student Information Form',endpoints:['GET /api/health','POST /api/apply','GET /api/admin/submissions']});
  if(req.method==='GET'){
